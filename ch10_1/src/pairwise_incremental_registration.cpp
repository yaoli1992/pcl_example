...
//申明一个结构体用来处理点云，处理过程中可以同时接受多个点云文件的输入
struct PCD
{
...
/** \brief Load a set of PCD files that we want to register together
  ...
  */
//检查每一个参数是否指向一个PCD文件，如果是，创建一个添加到点云矢量data中的PCD对象
void loadData (int argc, char **argv, std::vector<PCD, Eigen::aligned_allocator<PCD> > &models)
{
  std::string extension (".pcd");
  // Suppose the first argument is the actual test model
  for (int i = 1; i < argc; i++)  //第一个参数是命令本身，所以从第二个参数开始解析
  {
...
void pairAlign (const PointCloud::Ptr cloud_src, const PointCloud::Ptr cloud_tgt, PointCloud::Ptr output, Eigen::Matrix4f &final_transform, bool downsample = false)
{
  ...
  // Compute surface normals and curvature计算表面的法向量和曲率
  PointCloudWithNormals::Ptr points_with_normals_src (new PointCloudWithNormals);
  PointCloudWithNormals::Ptr points_with_normals_tgt (new PointCloudWithNormals);

  pcl::NormalEstimation<PointT, PointNormalT> norm_est;     
  pcl::search::KdTree<pcl::PointXYZ>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZ> ());
  norm_est.setSearchMethod (tree);           //点云法线估计对象
  norm_est.setKSearch (30);                  //设置估计时进行搜索用的k数
  
  norm_est.setInputCloud (src);
  norm_est.compute (*points_with_normals_src);     //分别估计源和目标点云法线
  ...
  // 将两个点云中的对应点对之间的 (src<->tgt) 最大距离值设置为 10cm  大于此值的点不考虑
  // Note: 根据数据集的大小来调整
  ...
  reg.setMaximumIterations (2);       //设置最大的迭代次数
  for (int i = 0; i < 30; ++i)      //手动迭代30次，每手动迭代一次，在配准结果视口对迭代的最新结果进行刷新显示
  {
   ...
   //如果迭代N次找到的变换和迭代N-1中找到的变换之间的差异小于ICP的变换收敛阀值，选择源和目标之间更靠近对应点距离
   //阀值来改善配准过程
  ...
  /*********************************************************************************
  一旦找到最优的变换，ICP返回的变换是从源点云到目标点云的变换矩阵，求逆变换得到从目标点云到
  源点云的变换矩阵，并应用到目标点云中，变换后的目标点云然后添加到源点云中，并且将点云和变换
  矩阵一起返回到主函数
  *********************************************************************************/
