...
{
  print_error ("Syntax is: %s -input:input.pcd  -grad_mag_thresh 10.0f -detect_thresh  0.75f  input_template.lmt\n", argv[0]);
...
{
  print_info (" linemod detected. For more information, use: %s -h\n", argv[0]);
  ...
  // Detect objects
  line_rgbd.setInputCloud (cloud); //Sets the input cloud with xyz point coordinates
  line_rgbd.setInputColors (cloud); //Sets the input cloud with rgb values.

  std::vector<LineRGBD<PointXYZRGBA>::Detection> detections;
  line_rgbd.detect (detections); //Applies the detection process and fills the supplied vector with the detection instances

  for (size_t i = 0; i < detections.size (); ++i)
  {
    const LineRGBD<PointXYZRGBA>::Detection & d = detections[i];
    const BoundingBoxXYZ & bb = d.bounding_box;  //The 3D bounding box of the detection
